---
Data: 2026-08-13
tags:
  - transpetro
  - edital
  - checklist
Tipo:
  - concurso
Fonte: https://concursos.cesgranrio.org.br/media/gpoweb-prd-hibrido3/eventos/22/conteudos/a4729a98-ca7f-44d4-be96-027d2d58b3dc.pdf?sv=2025-01-05&se=2036-08-10T14%3A01%3A46Z&sr=c&sp=r&sig=FRylEmYrLSBHv6dr9GT5mlfCRVunO37UCaS6YXWDWWg%3D
---
 ---
# Mapa do Edital — 94 subtópicos

## Cronograma

| Semanas | Blocos | Subtópicos |
| --- | --- | --- |
| 1–2 | Bloco 1 | 23 |
| 3–4 | Blocos 7 e 5 | 16 |
| 5–6 | Blocos 2, 3 e 4 | 14 |
| 7–9 | Bloco 9 | 21 |
| 10 | Bloco 6 | 11 |
| 11 | Bloco 8 | 9 |
| 12–14 | **Simulados** | — |
| 15 | Revisão de erros | — |

---

## Bloco 1 — Arquitetura de Dados (S1–2) · ~12 questões

- [x] 1.1 Modelagem conceitual, lógica e física
- [x] 1.2 Criação e alteração dos modelos lógico e físico
- [x] 1.3 Modelo relacional
- [x] 1.4 Normalização
- [x] 1.5 Integridade referencial
- [x] 1.6 Metadados
- [x] 1.7 Modelagem dimensional
- [x] 1.8 Avaliação de modelos de dados
- [x] 1.9 Engenharia reversa de modelos
- [x] 1.10 SQL
- [x] 1.11 DDL
- [x] 1.12 DML
- [x] 1.13 SGBD
- [x] 1.14 ACID
- [x] 1.15 Independência de dados
- [x] 1.16 Transações
- [x] 1.17 Performance de banco
- [x] 1.18 NoSQL
- [x] 1.19 Integração: ETL, transferência de arquivos
- [x] 1.20 Banco em memória
- [x] 1.21 Qualidade de dados e dados mestres
- [x] 1.22 Data Lakes e Big Data
- [x] 1.23 Relacional × multidimensional × documentos × grafos

## Bloco 7 — Análise de Dados e BI (S3) · ~7 questões

- [ ] 7.1 Dado, informação, conhecimento, inteligência
- [ ] 7.2 Business Intelligence
- [ ] 7.3 Mapeamento de fontes de dados
- [ ] 7.4 Dados estruturados e não estruturados
- [ ] 7.5 OLAP e operações
- [ ] 7.6 Data warehouse
- [ ] 7.7 Modelagem multidimensional
- [ ] 7.8 Relatórios e dashboards
- [ ] 7.9 Manipulação em planilhas
- [ ] 7.10 Geração de insights
- [ ] 7.11 BI na tomada de decisão

## Bloco 5 — Engenharia de Software (S4) · ~6 questões

- [ ] 5.1 Requisitos: levantamento, análise, gerenciamento
- [ ] 5.2 Ciclo de vida e paradigmas
- [ ] 5.3 Paradigma estruturado × orientado a objetos
- [ ] 5.4 Verificação, validação e teste
- [ ] 5.5 Ambientes de desenvolvimento

## Blocos 2 e 3 — Projetos e Produtos (S5) · ~6 questões

- [ ] 2.1 Scrum e Kanban
- [ ] 2.2 Gestão de projeto × gestão de produto
- [ ] 2.3 SAFe e ágil em escala
- [ ] 2.4 PMBOK 7ª edição
- [ ] 2.5 Projetos e a organização
- [ ] 2.6 Escritório de projetos (PMO)
- [ ] 2.7 Modelos e características
- [ ] 3. Processos, grupos de processos, áreas de conhecimento

## Bloco 4 — Gestão e Governança de TI (S6) · ~5 questões

- [ ] 4.5 ITIL e COBIT ⭐
- [ ] 4.6 LGPD — Lei 13.709 ⭐
- [ ] 4.1 Conceitos e mercado de TI
- [ ] 4.2 Economia da inovação
- [ ] 4.3 Perspectivas da tecnologia
- [ ] 4.4 Ciência, P&D e indústria

## Bloco 9 — Segurança da Informação (S7–9) · ~7 questões

**S7 — ataques e defesas**
- [ ] 9.1 Segurança física e lógica
- [ ] 9.2 Firewall, Proxy, IPS/IDS, DLP, CASB, SIEM, EDR, WAF
- [ ] 9.3 Malware: ransomware, vírus, worm, spyware, rootkit
- [ ] 9.4 Ataques: DDoS, SQLi, XSS, CSRF, Path Traversal
- [ ] 9.5 Desenvolvimento seguro, SAST/DAST/IAST
- [ ] 9.6 VPN
- [ ] 9.7 MDM

**S8 — identidade e resposta**
- [ ] 9.8 SSO
- [ ] 9.9 MFA
- [ ] 9.10 IAM, RBAC e ABAC
- [ ] 9.11 Resposta a incidente — NIST SP 800-61
- [ ] 9.12 Threat intel e threat hunting
- [ ] 9.13 Testes de penetração
- [ ] 9.14 Modelagem de ameaças — STRIDE

**S9 — normas e governança**
- [ ] 9.15 MITRE ATT&CK (só as táticas)
- [ ] 9.16 ISO 31000, ISO 22301, Sarbanes-Oxley
- [ ] 9.17 Políticas de segurança
- [ ] 9.18 Classificação de informações
- [ ] 9.19 ISO 27002, criptografia, certificado e assinatura digital
- [ ] 9.20 Segurança em nuvem
- [ ] 9.21 Segurança em IoT

## Bloco 6 — UX (S10) · ~4 questões

- [ ] 6.1 Acessibilidade e usabilidade
- [ ] 6.2 Histórias do usuário
- [ ] 6.3 Planejamento de interação web
- [ ] 6.4 Projeto centrado no usuário
- [ ] 6.5 Storytelling com dados
- [ ] 6.6 Relatórios e dashboards
- [ ] 6.7 Interoperabilidade entre navegadores
- [ ] 6.8 MVP
- [ ] 6.9 Prototipação
- [ ] 6.10 Design thinking
- [ ] 6.11 Personas

## Bloco 8 — Lógica Matemática (S11) · ~3 questões

- [ ] 8.1 Conectivos lógicos
- [ ] 8.2 Argumentos
- [ ] 8.3 Lógica sentencial
- [ ] 8.4 Lógica de predicados de 1ª ordem
- [ ] 8.5 Regras de formação de fórmulas
- [ ] 8.6 Sistemas dedutivos
- [ ] 8.7 Decidibilidade
- [ ] 8.8 Valores-verdade
- [ ] 8.9 Funções de avaliação

---

## Língua Portuguesa (30 min, seg/qua/sex)

- [ ] Compreensão de textos
- [ ] Ortografia oficial
- [ ] Coesão textual
- [ ] Significação das palavras
- [ ] Tempos e modos verbais
- [ ] Classes de palavras
- [ ] Coordenação e subordinação
- [ ] Pontuação
- [ ] Concordância verbal e nominal
- [ ] Regência verbal e nominal
- [ ] Crase
- [ ] Colocação pronominal

## Língua Inglesa (30 min, ter/qui)

- [ ] Compreensão de texto
- [ ] Itens gramaticais para compreensão semântica




| Rodada | Assuntos                                            | Denso?             | Por quê                                    |
| ------ | --------------------------------------------------- | ------------------ | ------------------------------------------ |
| **1**  | Scrum + Kanban + projeto × produto                  | Leve               | Você vive isso no trabalho e no WhatsBotAI |
| **2**  | SAFe + ágil em escala                               | **Denso, sozinho** | Novo, framework grande                     |
| **3**  | PMBOK — grupos de processos + áreas de conhecimento | **Denso, sozinho** | Muito vocabulário, números fixos           |
| **4**  | PMBOK — PMO + projetos na organização               | Leve               | Complemento do anterior, curto             |
| **5**  | ITIL                                                | **Denso, sozinho** | Ciclo de vida, decoreba pesada             |
| **6**  | COBIT                                               | **Denso, sozinho** | Domínios e siglas                          |
| **7**  | LGPD                                                | **Denso, sozinho** | Lei, cai muito                             |
| **8**  | Contexto de TI (mercado, inovação, P&D)             | Leve               | Decoreba rápida, item 4.1–4.4              |

